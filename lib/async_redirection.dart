...
/// This scenario demonstrates how to use redirect to handle a asynchronous sign-in flow.
///
/// The 'StreamAuth' is a mock of google_sign_in.
/// This example wraps it with an InheritedNotifier, StreamAuthScope, and relies on 'dependOnInheritedWidgetOfExactType'
/// to create a dependency between the notifier and go_router's parsing pipeline.
/// When StreamAuth broadcasts new event, the dependency will cause the go_router to re-parse the current url
...
/// An asynchronous log in services mock with stream similar
///
/// This class adds an artificial delay of 3 second when logging in an user,
///  and will automatically clear the login session after [refreshInterval]

class StreamAuth {
  /// Creates an [StreamAuth] that clear the current session in
  ...
  /// Checks whether current user is signed in with an artificial delay to mimic async operation.
